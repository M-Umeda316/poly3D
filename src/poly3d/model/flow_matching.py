...
class FlowMatching(nn.Module):
    ...
    Parameters
    ----------
    model      : LatentDiT  (zt, cond, t, batch, edge_index, z_sc) → z1_pred
    ...
    def loss(
        ...
        t = torch.rand(int(B), device=device) * self.t_max   # (B,)
        ...
        # 予測 (self-conditioning 込み)
        z1_pred = self.model.forward_with_selfcond(
            zt, cond, t, batch, edge_index, self.p_selfcond
        )

        # 損失: 1/(1-t)^2 * ||Z1 - Z1_pred||^2 の平均
        weight = 1.0 / (1.0 - t_node).pow(2).clamp(min=1e-4)
        diff_sq = (z1 - z1_pred).pow(2).mean(dim=-1)
        flow_loss = (weight * diff_sq).mean()
    ...
    def sample(
        ...
        for i in range(n_steps):
            ...
            # 1 回だけ forward (前ステップの z_sc を self-conditioning に使う)
            z1_pred = self.model.forward(zt, cond, t, batch, edge_index, z_sc)
